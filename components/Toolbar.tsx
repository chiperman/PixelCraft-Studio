...
export const Toolbar: React.FC<ToolbarProps> = ({
  ...
  return (
    <div
      className={`fixed z-50 w-72 top-4 bottom-4 left-4 flex flex-col transform transition-transform duration-300 ease-spring rounded-[1.5rem] glass-panel overflow-y-auto custom-scrollbar ${isSidebarOpen
          ...
          <div className="grid grid-cols-4 gap-2 place-items-center">
            {tools.map((tool) => (
              <button
                ...
                className={`w-11 h-11 rounded-xl flex justify-center items-center transition-all duration-200 group relative overflow-hidden ${activeTool === tool.id
            ...
            <div className="grid grid-cols-5 gap-3 w-fit mx-auto">
              {DEFAULT_PALETTE.map((color) => (
                <button
                  ...
                  className={`w-9 h-9 rounded-full border border-black/5 dark:border-white/10 transition-all duration-200 ${selectedColor === color
                      ? 'ring-2 ring-indigo-500 ring-offset-1 ring-offset-white dark:ring-offset-[#1c1c1e] scale-110 shadow-md z-10'
                      : 'hover:scale-110 hover:shadow-sm hover:z-10'
                    }`}
                  style={{ backgroundColor: color }}
