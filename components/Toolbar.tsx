...
export const Toolbar: React.FC<ToolbarProps> = ({
  ...
  return (
    <div
      className={`fixed z-50 w-72 top-4 bottom-4 left-4 flex flex-col transform transition-transform duration-300 ease-spring rounded-[1.5rem] glass-panel overflow-y-auto custom-scrollbar ${
        isSidebarOpen
          ? 'translate-x-0 shadow-2xl'
          : '-translate-x-[calc(100%+2rem)] md:translate-x-0 md:shadow-2xl'
      }`}
    >
      <div className="p-4 flex flex-col h-full gap-2">
        ...
        <div className="flex flex-col gap-2 mt-auto flex-shrink-0 pt-3 border-t border-slate-200/50 dark:border-white/5">
          <h3 className="text-xs uppercase font-bold text-slate-400 dark:text-slate-500 tracking-wider text-left pl-1 mb-0.5">
            {t.headers.actions}
          </h3>
          ...
          <div className="grid grid-cols-2 gap-2">
            <ActionCard icon={<FolderOpen size={22} />} label={t.actions.load} isFile>
            ...
            <ActionCard icon={<ImageIcon size={22} />} label={t.actions.importImg} isFile>
              <input
                type="file"
                className="hidden"
                accept="image/*"
                onChange={(e) => {
                  onImageUpload(e, 'convert');
                  if (window.innerWidth < 768) onCloseSidebar();
                }}
              />
            </ActionCard>

            <ActionCard icon={<Upload size={22} />} label={t.actions.refLayer} isFile>
              <input
                type="file"
                className="hidden"
                accept="image/*"
                onChange={(e) => {
                  onImageUpload(e, 'background');
                  if (window.innerWidth < 768) onCloseSidebar();
                }}
              />
            </ActionCard>
