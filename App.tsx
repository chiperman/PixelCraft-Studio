  ...
  return (
    <div className="relative flex-1 min-w-0" ref={containerRef}>
        <button 
            ...
            className={`w-full flex items-center justify-between px-3 bg-white/50 dark:bg-white/5 rounded-xl border border-white/20 dark:border-white/5 text-slate-700 dark:text-slate-200 transition-all active:scale-[0.98] hover:bg-white/70 dark:hover:bg-white/10 glass-panel ${className}`}
        >
                ...
                <div className="flex items-center gap-3 pl-2">
                    ...
                    <div className="relative z-50" ref={themeDropdownRef}>
                        <button 
                            ...
                            className={`h-10 w-10 flex items-center justify-center bg-white/50 dark:bg-white/5 rounded-xl border border-white/20 dark:border-white/5 text-slate-500 hover:text-indigo-500 dark:text-slate-400 dark:hover:text-white transition-colors hover:bg-white dark:hover:bg-white/10 shadow-sm ${isThemeDropdownOpen ? 'bg-white dark:bg-white/10 text-indigo-500 dark:text-white ring-2 ring-indigo-500/20' : ''}`}
                        >
                        ...
                        <div className="absolute right-0 top-full pt-2 w-40 animate-in fade-in zoom-in-95 duration-200">
                            <div className="glass-panel rounded-xl shadow-2xl overflow-hidden p-1">
                                <button onClick={() => { setState(s => ({...s, theme: 'light'})); setIsThemeDropdownOpen(false); }} className={`w-full text-left px-3 py-2 text-sm rounded-lg flex items-center gap-2 transition-colors ${state.theme === 'light' ? 'bg-indigo-500 text-white' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/10'}`}>
                                    <Sun size={16} /> {t.ui.lightMode}
                                </button>
                                <button onClick={() => { setState(s => ({...s, theme: 'dark'})); setIsThemeDropdownOpen(false); }} className={`w-full text-left px-3 py-2 text-sm rounded-lg flex items-center gap-2 transition-colors ${state.theme === 'dark' ? 'bg-indigo-500 text-white' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/10'}`}>
                                    <Moon size={16} /> {t.ui.darkMode}
                                </button>
                                <button onClick={() => { setState(s => ({...s, theme: 'system'})); setIsThemeDropdownOpen(false); }} className={`w-full text-left px-3 py-2 text-sm rounded-lg flex items-center gap-2 transition-colors ${state.theme === 'system' ? 'bg-indigo-500 text-white' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/10'}`}>
